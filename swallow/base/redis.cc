...
ReplyPtr Redis::cmd(const std::vector<std::string>& varg) {
  ...
  auto r = (redisReply*)redisCommandArgv(m_context.get(), varg.size(), &argv[0],
                                         &argvlen[0]);
  ReplyPtr smptr(r, freeReplyObject);
  if (r->type != REDIS_REPLY_ERROR) {
    return smptr;
...
bool Redis::connect() { return connect(m_host, m_port, m_timeout); }
...
int Redis::appendCmd(const std::vector<std::string>& varg) {
  ...
  return redisAppendCommandArgv(m_context.get(), varg.size(), &argv[0],
                                &argvlen[0]);
}

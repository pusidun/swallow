...
class MutexLock {
 public:
  MutexLock() { pthread_mutex_init(&m_mutex, nullptr); }
...
class RdLockGuard {
 public:
  explicit RdLockGuard(T& _rdlock) : m_rdlock(_rdlock) { m_rdlock.rdlock(); }

  ~RdLockGuard() { m_rdlock.unlock(); }
...
class WrLockGuard {
 public:
  explicit WrLockGuard(T& _wrlock) : m_wrlock(_wrlock) { m_wrlock.wrlock(); }

  ~WrLockGuard() { m_wrlock.unlock(); }

...
#define SWALLOW_LOG_LEVEL(logger, level)                                   \
  ...
  swallow::LogEventWrapper(                                                \
      swallow::LogEvent::ptr(new swallow::LogEvent(                        \
          logger, level, __FILE__, __LINE__, 0, 456, 123, 0, "thread 0"))) \
      .getSS()

#define SWALLOW_LOG_DEBUG(logger) \
  SWALLOW_LOG_LEVEL(logger, swallow::LogLevel::DEBUG)
#define SWALLOW_LOG_INFO(logger) \
  SWALLOW_LOG_LEVEL(logger, swallow::LogLevel::INFO)
#define SWALLOW_LOG_WARN(logger) \
  SWALLOW_LOG_LEVEL(logger, swallow::LogLevel::WARN)
#define SWALLOW_LOG_ERROR(logger) \
  SWALLOW_LOG_LEVEL(logger, swallow::LogLevel::ERROR)
#define SWALLOW_LOG_FATAL(logger) \
  SWALLOW_LOG_LEVEL(logger, swallow::LogLevel::FATAL)
...
#define SWALLOW_LOG_GET(logname) \
  swallow::LoggerMgr::getInstance()->getLogger(logname)

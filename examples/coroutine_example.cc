...
void foo2(void *arg) {
    ...
    SWALLOW_LOG_INFO(SWALLOW_LOG_ROOT()) << "thread " << *id << " before yield,state:" << co->getStat();
    co->yield();
...
void* foo(void* arg) {
    ...
    SWALLOW_LOG_INFO(SWALLOW_LOG_ROOT()) << "thread "<< *tid << " before first resume state:" << co->getStat();
    co->resume();
    SWALLOW_LOG_INFO(SWALLOW_LOG_ROOT()) << "thread "<< *tid << " before second resume state:" << co->getStat();
    co->resume();
